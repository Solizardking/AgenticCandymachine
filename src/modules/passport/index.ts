...
import {
    AgentDNA, ArtArtifact, TokenDeployResult, PassportBundle,
    PassportNFT, RecursiveMetadata, CapabilityType
} from "../../types/index.js";
import { sha256, buildPointerUri, nowUnix } from "../../utils/index.js";
...
export class PassportFactory {
    constructor(private config: PassportFactoryConfig) { }
    ...
    buildBundle(
        ...
        const capabilities: PassportNFT[] = [];
        const childrenPointers: any[] = []; // Metadata pointers

        for (const cap of dna.capabilities) {
            const capMint = Keypair.generate().publicKey;
            const capNFT: PassportNFT = {
